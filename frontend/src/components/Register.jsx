    ...
    const handleSubmit = async (e) => {
        ...
        if (password !== confirmPassword) {
            ...
            try {
                ...
                toast.error('There was an error registering: ' + (error.response ? error.response.data.message : error.message));
            }
        }
    };

    return (
        <div className="flex items-center justify-center min-h-screen" style={{ backgroundImage: `url(${loginImage})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(-0.5px)' }}>
            <div className="p-2 mt-20 rounded-lg max-h-dvh w-full max-w-md shadow-[8px_8px_10px_#5f9ea0]" style={{ backgroundColor: 'rgba(255, 255, 255, 0.6)' }}>

                <h2 className="text-2xl font-bold text-center text-gray-800">Register</h2>
                <form onSubmit={handleSubmit} className="space-y-2 space-x-2 p-4">
                    <div className='flex gap-4'>
                        <div>
                            ...
                            <input
                                ...
                                className="w-full p-2 border border-gray-300 rounded mb-4 focus:outline-none focus:ring-2 focus:ring-[#1c4243]"
                            />
                    ...
                    <div className="text-center">
                        Already have an account? <Link to="/login" className="text-indigo-600 hover:text-indigo-800 hover:underline">Login</Link>
                    </div>
                </form>
            </div>
        </div>
